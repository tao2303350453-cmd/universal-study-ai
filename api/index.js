 ...
 * 使用 Vercel Node.js 运行时 - CommonJS syntax
 */
...
async function callDeepSeek(question, context) {
  if (!DS_KEY) return 'DeepSeek API 未配置。请设置 DEEPSEEK_API_KEY 环境变量。';
...
async function parseFormData(req) {
  return new Promise((resolve) => {
    ...
    bb.on('close', () => { if (!resolveCalled) { resolveCalled = true; resolve(result); } });
    req.pipe(bb);
  });
}
  ...
  try {
    ...
    if (path.startsWith('api/nodes/') && method === 'DELETE') {
      ...
      if (Array.isArray(allNodes)) {
        const map = {};
        allNodes.forEach(n => { if (n.parent_id) (map[n.parent_id] = map[n.parent_id] || []).push(n.id); });
        const queue = [nodeId];
        while (queue.length) { const pid = queue.pop(); (map[pid]||[]).forEach(cid => { toDelete.push(cid); queue.push(cid); }); }
      }
    ...
    if (path === 'api/documents/upload' && method === 'POST') {
      ...
      if (!nodeId || !file) return res.status(400).json({ error: '缺少参数' });
